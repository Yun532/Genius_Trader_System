...
interface Props {
  ...
  onOpenDeep?: (newsId: string, date: string) => void;
...
function pct(v: number | null) {
  ...
  const pctVal = v * 100;
  const color = pctVal > 0 ? '#ff3d8b' : pctVal < 0 ? '#00e5a8' : '#888';
  return <span style={{ color, fontWeight: 600 }}>{pctVal > 0 ? '+' : ''}{pctVal.toFixed(2)}%</span>;
}

export default function NewsPanel({ symbol, hoveredDate, onFindSimilar, onOpenDeep, highlightedNewsId, isLocked, onUnlock, highlightedCategoryIds }: Props) {
  ...
  if (!displayDate) {
    return (
      <div className="news-panel">
        ...
        <div className="news-empty">点击 K 线上的事件点查看新闻、公告、财报或资金资讯</div>
      </div>
    );
  }
  ...
  return (
    <div className="news-panel">
      ...
      {loading && news.length === 0 ? (
        ...
        <div className="news-empty">这一天暂无事件</div>
      ) : (
                  ...
                  <div className="returns-chips">
                    ...
                    {onOpenDeep && (
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); onOpenDeep(item.news_id, item.trade_date); }}
                      >
                        深挖
                      </button>
                    )}
